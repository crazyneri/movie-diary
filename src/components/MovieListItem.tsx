...
export default function MovieListItem({movie, open}: {movie: MovieDetail, open: (activeMovie: MovieDetail, action:string) => void} )
{
    ...
    const handleClick = async (e:React.MouseEvent<HTMLSpanElement>) => {
        e.preventDefault();
        const updatedMovie = {...movie, "isWatched": !movie.isWatched};
    ...
    <div className={formChild + ' article'}>
            <div className="flex justify-between mb-[0.5rem]">
                <div className="flex items-center">
                    <h2 className={itemTitle +' mr-[.5rem]'}>{movie && movie.title}</h2>
                ...
                <span onClick={handleClick} className={itemBadge + ' cursor-pointer article__hovered-text'}>{movie.isWatched ? 'Watched' : 'Mark as watched' }</span>
            </div>
