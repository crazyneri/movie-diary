...
export default function MovieList()
{
    ...
    const handleActiveMovie = (movie:MovieDetail) => {
    ...
    const renderedMovies = movies.map((movie, index) => {
        return <MovieListItem key={index} movie={movie} open={handlePopup} activeMovie={handleActiveMovie}/>
    })
    ...
    return <div className={containerForm + ' flex flex-col gap-[1rem]'}>
        {container}
        {isOpen && <Modal onClose={handlePopup}>
            ...
            <div className={containerFlex + ' gap-[1rem] max-w-lg mx-auto'}>
                <Button type="cancel" onClick={handlePopup}>Cancel</Button>
